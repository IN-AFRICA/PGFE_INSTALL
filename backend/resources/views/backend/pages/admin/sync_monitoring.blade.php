<x-layouts.modules-layout :breadcrumbs="[['label' => 'Dashboard', 'url' => route('admin.dashboard')], ['label' => 'Synchronisation', 'url' => '#']]">
    <x-backend.pages.module-sidebar-layout 
        logoIcon="lucide:refresh-cw"
        :menuItems="[
            ['label' => 'Moniteur Flux', 'icon' => 'lucide:activity', 'url' => route('admin.sync.monitoring'), 'route_prefix' => 'admin.sync.monitoring'],
            ['label' => 'Historique Sync', 'icon' => 'lucide:history', 'url' => '#', 'active' => false],
            ['label' => 'Configuration', 'icon' => 'lucide:settings-2', 'url' => '#', 'active' => false],
        ]"
    >
        <div class="space-y-10">
            ...
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white dark:bg-gray-900 p-8 rounded-[2.5rem] shadow-xl shadow-gray-200/50 dark:shadow-none border border-gray-100 dark:border-gray-800">
                ...
                <div class="flex flex-col md:items-end gap-3">
                    @if(session('status'))
                        <div class="px-4 py-2 rounded-2xl bg-emerald-50 text-emerald-700 border border-emerald-100 text-xs font-medium">
                            {{ session('status') }}
                        </div>
                    @else
                    ...
                    <form method="POST" action="{{ route('admin.sync.run') }}">
                        @csrf
                        <button type="submit" class="inline-flex items-center gap-2 px-5 py-2 rounded-2xl bg-blue-600 text-white text-xs font-black tracking-widest uppercase shadow-lg shadow-blue-600/30 hover:bg-blue-700">
